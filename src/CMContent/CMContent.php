...
class CMContent extends CObject implements IHasSQL, ArrayAccess {
  ...
  public function Init() {
    try {
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('htmlpurify', 'page', 'Page with HTMLPurify', "This is a demo page with some HTML code intended to run through <a href=''>HTMLPurify</a>. Edit the source and insert HTML code and see if it works.\n\n<b>Text in bold</b> and <i>text in italic</i> and <a href='http://dbwebb.se'>a link to dbwebb.se</a>", 'htmlpurify', $this->user['id']));
  ...
  public static function Filter($data, $filter) {
    switch($filter) {
      ...
      case 'htmlpurify': $data = nl2br(makeClickable(CHTMLPurifier::Purify($data))); break;
      case 'bbcode': $data = nl2br(bbcode2html($data)); break;
      case 'plain': 
