...
class CMUser extends CObject implements IHasSQL, ArrayAccess, IModule {
  ...
  public static function SQL($key=null) {
    $queries = array(
      ...
      'export table user'       => "SELECT * FROM User;",
      'export table group'      => "SELECT * FROM Groups;",
      'export table user2group' => "SELECT * FROM User2Groups;",
      ...
      'update profile'          => "UPDATE User SET name=?, email=?, meta=?, updated=datetime('now') WHERE id=?;",
  ...
  public function Login($akronymOrEmail, $password) {
    ...
    unset($user['salt']);
    unset($user['password']);
    $user['meta'] = isset($user['meta']) ? unserialize($user['meta']) : array();
  ...
  public function Save() {
    $this->db->ExecuteQuery(self::SQL('update profile'), array($this['name'], $this['email'], serialize($this['meta']), $this['id']));
    $this->session->SetAuthenticatedUser($this->profile);
  ...
  /**
   * User changes own meta information.
   *
   * @param string $acronym current user according to controller
   * @param array $meta with entries to update.
   * @return boolean true if success else false.
   */
  public function ChangeOwnMeta($acronym, $meta) {
   ...
   *
   * @param string $key for the meta value to get.   
